...
<div class="row g-3 mx-max-md-0 mb-3">
    <section class="col-lg-8 px-max-md-0">
                            ...
                            <div
                                ...
                                @else
                                    ...
                                    <div class="d-flex gap-2">
                                        ...
                                        {{-- {{dd($verify_status['minimum_order_amount'],$verify_status['amount'])}} --}}
                                            @if ($verify_status['minimum_order_amount'] > $verify_status['amount'])
                                                <span class="pl-1 text-danger pulse-button minimum-order-amount-message" data-toggle="tooltip"
                                                      data-placement="right"
                                                      data-title="{{ translate('minimum_Order_Amount') }} {{ webCurrencyConverter(amount: $verify_status['minimum_order_amount']) }} {{ translate('for') }} @if($cartItem->seller_is=='admin') {{getWebConfig(name: 'company_name')}} @else {{ \App\Utils\get_shop_name($cartItem['seller_id']) }} @endif"
                    ...
                    <table
                        ...
                        @foreach($group as $cart_key=>$cartItem)
                            ...
                            <tr>
                                <td class="__w-45">
                                    <div class="d-flex gap-3 align-items-center">
                                        <input style="display: none" type="checkbox" class="shop-item-check shop-item-check-desktop" value="{{ $cartItem['id'] }}" {{ $cartItem['is_checked'] ? 'checked' : '' }}>
        ...
        @foreach($cart as $group_key => $group)
            <div class="cart_information mb-0 pb-0  d-lg-none">
                    <?php
                    $isPhysicalProductExist = false;
                    $total_shipping_cost = 0;
                    foreach ($group as $row) {
                ...
                @foreach($group as $cart_key=>$cartItem)
                    ...
                    <div
                        ...
                        <div class="d-flex gap-3 align-items-center">
                            <input type="hidden" class="shop-item-check shop-item-check-mobile" value="{{ $cartItem['id'] }}" {{ $cartItem['is_checked'] ? 'checked' : '' }}>
                ...
                <div class="mb-lg-3">
                    <div class="row">
                        <div class="col-6">
                            ...
                            <input class="form-control " id="order_note"
                                      name="order_note" value="{{ session('order_note')}}">
                        </div>
...
<script>
    window.onload = function() {
      let id = 9;
      let cartGroupId = "all_cart_group";
