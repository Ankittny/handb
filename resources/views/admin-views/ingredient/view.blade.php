                ...
                <div class="card">
                    <div class="card-body text-start">
                        <form action="{{ route('admin.category.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="organic_status" value="1">
                            ...
                            <div class="row">
                                    ...
                                    <div>
                                        @foreach($languages as $lang)
                                            <div
                                                ...
                                                id="{{ $lang}}-form">
                                                <label class="title-color">{{ translate('ingredient_Name') }}<span
                                                        class="text-danger">*</span> ({{strtoupper($lang) }})</label>
                                                <input type="text" name="name[]" class="form-control category-title-name"
                                                       placeholder="{{ translate('ingredient_Category') }}" {{ $lang == $defaultLanguage? 'required':''}}>
                                            </div>
                              ...
                              <div class="form-group">
                                        ...
                                        <input type="text" name="meta_title" class="form-control" placeholder="{{ translate('enter_meta_title') }}">
                                    </div>
                                    <div class="form-group">
                                        <label class="title-color" for="meta_description">{{ translate('Meta Description') }}</label>
                                        <textarea name="meta_description" class="form-control" placeholder="{{ translate('enter_meta_description') }}"></textarea>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group pt-2">
                                        ...
                                        <textarea class="summernote {{ $lang == $defaultLanguage ? 'content-writing-area' : '' }}" name="content_writing_area">{{ old('content_writing_area') }}</textarea>
                                    </div>
                              
                                    <div class="from_part_2">
                                        <label class="title-color">{{ translate('ingredient_Logo') }}</label>
                ...
                <div class="card">
                    ...
                    <div class="table-responsive">
                        <table
                            ...
                            <tr>
                                <th>{{ translate('ID') }}</th>
                                <th class="text-center">{{ translate('ingredient_Image') }}</th>
                                     ...
                                     <!-- <td class="text-center">
                                        <form action="{{ route('admin.category.organicstatus') }}" method="post" id="organic-status{{ $category['id'] }}-form">
                                            ...
                                            <label class="switcher mx-auto">
                                                <input type="checkbox" class="switcher_input toggle-switch-message"
                                                       name="organic_status"
                                                       id="organic-status{{ $category['id'] }}" value="1"
                                                       {{ $category['organic_status'] == 1 ? 'checked' : '' }}
                                                       data-modal-id="toggle-organic-modal"
                                                       data-toggle-id="organic-status{{ $category['id'] }}"
                                                       data-on-image="category-status-on.png"
                                                       data-off-image="category-status-off.png"
                                                       data-on-title="{{ translate('Want_to_Turn_ON').' '.$category['defaultname'].' '. translate('organic') }}"
                                                       data-off-title="{{ translate('Want_to_Turn_OFF').' '.$category['defaultname'].' '.translate('organic') }}"
                                                       data-on-message="<p>{{ translate('if_enabled_this_category_will_be_visible_from_the_category_wise_product_section_in_the_website_and_customer_app_in_the_homepage') }}</p>"
                                                       data-off-message="<p>{{ translate('if_disabled_this_category_will_be_hidden_from_the_category_wise_product_section_in_the_website_and_customer_app_in_the_homepage') }}</p>">
                                                <span class="switcher_control"></span>
                                            </label>
                                        </form>
                                    </td> -->
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-10">
                                            <a class="btn btn-outline-info btn-sm square-btn "
                                               ...
                                               href="{{ route('admin.category.ingredient-update',[$category['id']]) }}">
                                                <i class="tio-edit"></i>
                                            </a>
