                ...
                <div class="card-body">
                    @foreach($languages as $language)
                        ...
                        <div class="{{ $language != 'en'? 'd-none':''}} form-system-language-form" id="{{ $language}}-form">
                           ...
                           <div class="form-group pt-4">
                                <label class="title-color">{{ translate('how_to_use') }}
                                    ({{strtoupper($language) }})</label>
                                <textarea name="how_to_use" class="summernote"
                    ...
                    <div class="row align-items-end">
                      ...
                      <div class="col-md-6 col-lg-4 col-xl-3">
                            <div class="form-group">
                                ...
                                @php
                                    $Hsncode = App\Models\Hsncode::all(); // Fetch data from the model
                                @endphp
                                <select class="js-select2-custom form-control action-get-request-onchange" name="hsn_code_under_gst" id="hsn_code_under_gst" onchange="handleSelectChange()">
                                   <option value="">{{ $product->hsn_code_under_gst }}</option>
                                    @foreach($Hsncode as $hsn)
                                    <option value="{{ $hsn->hsn_code_under_gst }}" <?php if($hsn->hsn_code_under_gst == $product->hsn_code_under_gst) echo 'selected'; ?>>{{ $hsn->hsn_code_under_gst." ".$hsn->description }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <div class="row align-items-end">
                        <div class="col-md-12">
                            @php
                                $selectedIngredients = [];
                                if ($product->ingredients_id != null) {
                                    $selectedIngredients = json_decode($product->ingredients_id, true);
                                }
                            @endphp
                    ...
                    <div class="row">
                        <div class="col-md-8">
                          ...
                          <div class="form-group">
                                ...
                                <input type="text" name="keywords" value="{{$product->keywords}}" class="form-control" placeholder="{{ translate('enter_keywords') }}">
                            </div>
                            <div class="form-group">
                                <label class="title-color" for="alt_tag">{{ translate('Alt Tag') }}</label>
                                <input type="text" name="alt_tag" value="{{$product->alt_tag}}" class="form-control" placeholder="{{ translate('enter_alt_tag') }}">
                            </div>
                        </div>
...
<script>
    function handleSelectChange() {
        ...
        $.ajax({
            url: "{{ route('vendor.products.gethsncode') }}", // Correctly rendered URL in JavaScript
            type: 'POST',
            headers: {
